...
interface Props {
  ...
  logger: Logger;
  timeout: number;
  onError(error: AnyObject): Error;
}

export function createInvokeAction({ socketRef, logger, responseCache, timeout, onError }: Props) {
    ...
    const promise = new Promise((resolve, reject) => {
      ...
      socket.timeout(timeout).emit('invokeAction', action, (connectionError: Error, { response, canBeCached }: InvokeActionMessageResponse, serverError?: AnyObject) => {
        if (connectionError || serverError) {
          const error = onError(serverError ?? connectionError);
